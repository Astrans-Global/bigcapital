 ...
 * invoices. Residual (1–2 invoices delayed past 60 days, not C/D) is B,
 * because that customer is not "fully cleared within 60 days" (A).
 */
export function computeCustomerRiskCategory(
  ...
  const delayedOverGrace = open.filter(
  ...
  if (delayedOverGrace >= RISK_B_MIN_DELAYED_INVOICES) {
    return 'B';
  }
  if (delayedOverGrace >= 1) {
    return 'B';
  }
  return 'A';
}
